...
function App() {
  const [screen, setScreen] = useLocalState('lp-app-screen', 'home', ['lp2-screen', 'lp3-screen', 'lp-screen']);
  const [area, setArea] = useLocalState('lp-app-area', 'outer', ['lp2-area', 'lp3-area', 'lp-area']);
  const [timeBlock, setTimeBlock] = useLocalState('lp-app-time-block', 'afternoon', ['lp2-time-block', 'lp3-time-block', 'lp-block']);
  const [boatId, setBoatId] = useLocalState('lp-app-boat-id', '1', ['lp2-boat-id', 'lp3-boat-id', 'lp-selected-boat', 'lp-boat-id']);
  const [communityMode, setCommunityMode] = useLocalState('lp-app-community-mode', 'similar', ['lp2-community-mode', 'lp3-community-mode']);
  const [units, setUnits] = useLocalState('lp-app-units', 'imperial', ['lp2-units', 'lp3-units']);
  const [boats, setBoats] = useLocalState('lp-app-boats', DEFAULT_BOATS, ['lp2-boats', 'lp3-boats', 'lp-boats']);
  const [pendingTrips, setPendingTrips] = useLocalState('lp-app-pending-trips', DEFAULT_PENDING, ['lp2-pending-trips', 'lp3-pending-trips', 'lp-pending']);
  const [completedTrips, setCompletedTrips] = useLocalState('lp-app-completed-trips', [], ['lp2-completed-trips', 'lp3-completed-trips', 'lp-doneTrips']);
  const [selectedPendingId, setSelectedPendingId] = useLocalState('lp-app-selected-pending', 'p1', ['lp2-selected-pending', 'lp3-selected-pending-id']);
  const [tripDate, setTripDate] = useLocalState('lp-app-trip-date', todayString(), ['lp2-trip-date', 'lp3-trip-date', 'lp-date']);
  const [email, setEmail] = useLocalState('lp-app-email', '', ['lp2-email', 'lp3-email']);
  const [isLoggedIn, setIsLoggedIn] = useLocalState('lp-app-logged-in', false, ['lp2-is-logged-in', 'lp3-is-logged-in']);
